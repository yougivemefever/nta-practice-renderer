...
app.controller("appCtrl", function($scope,$window) {
    ...
    $window.submitSelection = function()
    {
      ...
      var correctAnswer = window.CommonAndroidAPI.getCorrectAnswer();
        $scope.answerPage = true;
    ...
    $scope.isMatrixAnswerChecked = function(header,header2)
    {
        if($scope.question.category == "Matrix")
        {
            ...
            if (  $scope.answerPage){
              corrAns = window.CommonAndroidAPI.getCorrectAnswer();
            }
            if(corrAns)
            {
